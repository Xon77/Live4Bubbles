CompressorUIX : UIFactories {
	...
	*new { |initHandler, initOrbits|
	...
	createUI {
		...
		thresholdLabel = StaticText.new.string_("% dB".format(defaultParentEventDict[\cpThresh])).font_(Font.default.size_(10)).fixedWidth_(40 /*60*/).align_(\center);
		// Adding (Font.default.size_(11)) to change Font and fixedWidth to optimize space
		...
		compressorElements.put(\cpThresh,
			Dictionary.newFrom([
				...
				, \title, StaticText.new.string_("T").align_(\center).maxHeight_(/*15*/ 10).maxWidth_(30)
		...
		compressorElements.put(\gainReduction,
			Dictionary.newFrom([
				...
				, \title, StaticText.new.string_("GR").align_(\center).maxHeight_(/*15*/ 10)
			])
		);
		...
		uiKnobFactories.knobWithValueLabelFactory2(
			...
			"%" // "%Hz" "%Freq" // Withdraw Hz to optimze space
		);
			...
			HLayout( // Added
				...
				VLayout(
					...
					, HLayout(
						...
						, compressorFeedback
						// VLayout beginning with \cpKnee below
						, VLayout(
