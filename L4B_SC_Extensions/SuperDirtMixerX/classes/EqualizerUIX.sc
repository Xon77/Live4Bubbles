EqualizerUIX : UIFactories{ // WARNING: keyword arg 'uiCallback' not found in call to Meta_EQui:new ?

	var handler;
	var orbits;
	var controlBusses;
	var container;
	var eqView; //, freqScope; replaced by ~tidalFreqScope
	var activeOrbit;
	...
	*new { |initHandler, initOrbits, initControlBusses, initContainer|
	...
	handleEvent { |eventName, eventData|
		if (eventName == \setActiveOrbit, {
			...
			/*freqScope*/ ~tidalFreqScope.inBus = activeOrbit.dryBus;
		});
	...
	disableUI {
		...
		p.hiPeakBypass = 1;
		p.hiShelfBypass = 1;
		p.loPassBypass = 1 /*1*/; // Error otherwise - Pourrait être mis sur d'autres paramètres si necessaire
...
/*this.eqFilterButtonFactory(\hiPassBypass, "../assets/images/highPass.svg".resolveRelative);
		...
		).spacing_(5).margins_(5); // added

		~tidalFreqScope = FreqScopeView(userView, Rect(0,0, userView.bounds.width, userView.bounds.height));
		~tidalFreqScope.freqMode = 1;
		~tidalFreqScope.dbRange = 86;
		~tidalFreqScope.active_(true); // turn it on the first time;
		~tidalFreqScope.style_(1);
		~tidalFreqScope.waveColors = [Color.new255(235, 235, 235)];
		~tidalFreqScope.background = Color.new255(255, 255, 255);
		~tidalFreqScope.fill = true;
		~tidalFreqScope.inBus = orbits[0].dryBus;
